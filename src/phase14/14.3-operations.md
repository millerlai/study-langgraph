...
1. Client 發送 Background Run 請求 + webhook URL
2. Agent Server 返回 202 Accepted（run_id）
3. Client 可以去做其他事
4. Agent Server 在背景執行 graph
5. 執行完成後，Agent Server POST webhook
6. Webhook 端處理完成通知

不推薦：
- 用 polling 查詢 run 狀態（浪費資源）
- 用同步 wait 等待長時間任務（阻塞連線）
```
