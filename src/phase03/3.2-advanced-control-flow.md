...
```
                    ...
                    │      └───────────┘        │
┌───────┐   ┌───────┴──┐                        │   ┌─────────┐   ┌─────┐
│ START │──>│ prepare  │ (同一 super-step 平行)  ├──>│ combine │──>│ END │
└───────┘   └───────┬──┘                        │   └─────────┘   └─────┘
                    │      ┌───────────┐        │
                    └─────>│  fetch_b  │────────┘
                           └───────────┘
...
```
                         ...
                         │    └───────────────┘   │
┌───────┐  ┌─────────┐  │    ┌───────────────┐   │   ┌─────────┐  ┌─────┐
│ START │─>│ planner │──┼───>│ worker("主題2")│───┼──>│ collect │─>│ END │
└───────┘  └─────────┘  │    └───────────────┘   │   └─────────┘  └─────┘
             (Send x N)  │    ┌───────────────┐   │
                         └───>│ worker("主題3")│───┘
                              └───────────────┘
...
```
super-step 0   super-step 1   super-step 2   ...   super-step N
┌──────────┐   ┌──────────┐   ┌──────────┐         ┌──────────────────┐
│ Node A   │──>│ Node B   │──>│ Node A   │──> ... ──│ GraphRecursionError!
└──────────┘   └──────────┘   └──────────┘         └──────────────────┘
                                                    (超過 recursion_limit)
```
