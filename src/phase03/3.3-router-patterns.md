...
```mermaid
flowchart LR
    START([ START ]):::term --> classify

    classify["**classify**\n─────────────\n規則分類"]:::node

    classify -->|type = 'tech'   | tech
    classify -->|type = 'billing'| billing
    classify -->|type = 'other'  | general
    ...
    respond["**respond**"]:::node --> END([ END ]):::term

    classDef term  fill:#e8e6ff,stroke:#6c63d4,color:#3b3480,rx:20
    classDef node  fill:#f5f4ff,stroke:#9b93e8,color:#2d2a5e
```
