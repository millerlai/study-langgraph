...
以下範例模擬 LLM 的路由決策（不需要實際的 LLM API key）。在實際應用中，你會用真正的 LLM 來做分類，可以參考 [3.3-example-llm-router-V2.py](./3.3-example-llm-router-V2.py)。
...
# [multi_router] 查詢='研究 Python 的非同步程式設計，並寫一篇分析報告' -> 分派給: ['research', 'write'] (理由: 需要先研究調查Python非同步程式設計的相關資料，然後撰寫分析報告)
...
# === 最終答案 ===
# 路由: ['research', 'write']
# 理由: 需要先研究調查Python非同步程式設計的相關資料，然後撰寫分析報告
# 答案:
# 基於研究結果，Python 非同步程式設計是一個以 asyncio 框架為核心的重要技術領域，透過 async/await 語法實現協程機制，能在單執行緒中高效處理多個 I/O 密集型任務。建議的分析報告應包含三個主要部分：首先介紹非同步程式設計的基礎概念與應用場景，接著深入分析技術實現細節包括事件循環機制和主要框架比較，最後透過實際案例評估效能優勢並討論最佳實踐。這種程式設計方式特別適用於 Web 開發和 API 服務等需要處理大量並發連線的場景，但需要注意其學習曲線較陡峭且除錯複雜度較高的特點。報告應著重於實務應用價值，幫助開發者理解何時以及如何有效運用非同步程式設計技術。
