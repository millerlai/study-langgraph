...
```mermaid
graph TD
    subgraph MS["Memory Store（跨 Thread 的長期記憶）"]
        NS["namespace: ('user-001','prefs')\n喜歡日本料理 / 住在台北 / 偏好簡潔回覆"]
    end

    MS --> T1
    MS --> T2

    subgraph T1["Thread-1（對話 A）"]
        T1C["Checkpointer（短期記憶）\n討論天氣..."]
    end

    subgraph T2["Thread-2（對話 B）"]
        T2C["Checkpointer（短期記憶）\n推薦餐廳..."]
    end
```
...
graph TD
    ...
    Store --> NS1["namespace:\n('user-001', 'memories')"]
    NS1 --> K1["key: 'mem-abc123'\nvalue: {'{food: 日本料理, city: 台北}'}"]
    NS1 --> K2["key: 'mem-def456'\nvalue: {'{hobby: 攝影}'}"]
    NS1 --> K3["key: 'mem-ghi789'\nvalue: {'{language: 繁體中文}'}"]

    Store --> NS2["namespace:\n('user-001', 'preferences')"]
    NS2 --> K4["key: 'ui-settings'\nvalue: {'{theme: dark, language: zh-TW}'}"]

    Store --> NS3["namespace:\n('user-002', 'memories')"]
    NS3 --> K5["key: 'mem-xyz001'\nvalue: {'{food: 義大利菜}'}"]

    Store --> NS4["namespace:\n('global', 'prompts')"]
    NS4 --> K6["key: 'system-v1'\nvalue: {'{prompt: 你是一個友善的助理...}'}"]
```
...
```mermaid
graph LR
    subgraph 編譯["graph = builder.compile(...)"]
        CP["checkpointer=InMemorySaver()\n(短期記憶)"]
        ST["store=InMemoryStore()\n(長期記憶)"]
    end

    subgraph 執行["graph.invoke(...)"]
        TH["config: thread_id='t1'\n(短期記憶 key)"]
    end

    subgraph Node["在 node 中存取"]
        ND["def my_node(state, *, store: BaseStore):\n    memories = store.search(namespace)"]
    end
...
| 搜尋方式 | API | 說明 |
|----------|-----|------|
| **普通搜尋** | `store.search(namespace, filter={"type": "food"})` | 所有 type="food" 的記憶（不排序） |
| **語意搜尋** | `store.search(namespace, query="使用者喜歡吃什麼？")` | 按語意相關度排序 |
...
|-------------|---------------|----------|
| **語意記憶** (Semantic) | 使用者事實/偏好，如：喜歡日本料理 | Store: `("user","facts")` / Profile 或 Collection |
| **情節記憶** (Episodic) | 過去的互動經驗，如：上次查詢了天氣 | Store: `("user","episodes")` / Few-shot 範例 |
| **程序性記憶** (Procedural) | Agent 的行為規則，如：回覆要簡潔 | Store: `("agent","rules")` / 可修改的 System Prompt |
