...
┌─────────────────────────┐      ┌─────────────────────────┐
│ User: 你好               │      │ User: 天氣如何？         │
│ AI:   你好！有什麼需要？  │      │ AI:   請問哪個城市？     │
│ User: 我叫小明           │      │ User: 台北              │
│ AI:   小明你好！         │      │ AI:   台北今天晴天       │
│ ─────────────────────── │      │ ─────────────────────── │
│ State 透過 Checkpointer  │      │ State 透過 Checkpointer  │
│ 持久化在資料庫中          │      │ 持久化在資料庫中          │
└─────────────────────────┘      └─────────────────────────┘
        │                                │
        v                                v
...
```
invoke(msg, thread_id="001")     invoke(msg, thread_id="001")
        │                                │
        v                                v
  ┌───────────┐                    ┌───────────┐
  │  載入上次   │                    │  載入上次   │
...
裁剪前                              裁剪後（保留最近 4 條）
┌──────────────────────┐           ┌──────────────────────┐
│ [0] System: 你是助理  │           │ [0] System: 你是助理  │  <-- 保留系統訊息
│ [1] User: 你好       │  ──X──   │ [1] User: 寫詩       │
│ [2] AI: 你好！       │  ──X──   │ [2] AI: 好的，這首... │
│ [3] User: 寫故事     │  ──X──   │ [3] User: 改成五言    │
│ [4] AI: 從前從前...   │  ──X──   │ [4] AI: 春風送暖...   │
│ [5] User: 寫詩       │           └──────────────────────┘
│ [6] AI: 好的，這首... │
│ [7] User: 改成五言    │
│ [8] AI: 春風送暖...   │
...
```
⚠️ 刪除訊息時必須確保剩餘的訊息歷史仍然有效！

常見陷阱：

1. 某些 LLM Provider 要求訊息歷史以 user 訊息開頭
   ┌─────────────────────┐
   │ ❌ [AI] 你好！       │  <-- 開頭是 AI 訊息 → 可能報錯
   │ [User] 幫我寫程式    │
   └─────────────────────┘

2. 帶有 tool_calls 的 AI 訊息必須有對應的 Tool 結果
   ┌───────────────────────────┐
   │ ❌ [AI] tool_call: search │  <-- 沒有對應的 tool 結果
   │ [User] 下一步呢？         │      → 會造成錯誤
   └───────────────────────────┘
