...
```mermaid
graph LR
    subgraph 不用Streaming["不用 Streaming"]
        A1["使用者"] --> A2["等待 5 秒..."] --> A3["完整回應一次顯示"]
    end

    subgraph 使用Streaming["使用 Streaming"]
        B1["使用者"] --> B2["token1"] --> B3["token2"] --> B4["token3 ..."] --> B5["逐步顯示"]
    end
```
...
```mermaid
graph TD
    subgraph GS["graph.stream()"]
        A["values\n每步完整 State"]
        B["updates\n僅變更的 key"]
        C["messages\nLLM token"]
        D["custom\n自定義資料"]
        E["checkpoints / tasks / debug\n進階除錯模式"]
    end
...
| 格式 | stream_mode | 回傳結構 |
|------|-------------|----------|
| **v1（預設）** | `"updates"` | `{"node_name": {更新的欄位}}` |
| **v1（預設）** | `"values"` | `{完整 State}` |
| **v2（推薦）** | `"updates"` | `{"type": "updates", "data": {"node_name": {更新}}}` |
| **v2（推薦）** | `"values"` | `{"type": "values", "data": {完整 State}}` |
...
| chunk | 階段 | items | count |
|-------|------|-------|-------|
| chunk 1 | Input State | `[初始]` | 0 |
| chunk 2 | step_a 後 | `[初始, A]` | 1 |
| chunk 3 | step_b 後 | `[初始, A, B]` | 2 |
| chunk 4 | step_c 後 | `[初始, A, B, C]` | 3 |
...
| chunk | values 模式（完整快照） | updates 模式（僅變更） |
|-------|-------------------------|------------------------|
| 1 | `{items: [初始], count: 0}` | (無初始 chunk) |
| 2 | `{items: [初始, A], count: 1}` | `{step_a: {items: [A], count: 1}}` |
| 3 | `{items: [初始, A], count: 1, metadata: "step_b 完成"}` | `{step_b: {metadata: "..."}}` |
| 4 | `{items: [初始, A, C], count: 2, metadata: "全部完成"}` | `{step_c: {items: [C], count: 2, metadata: "全部完成"}}` |

> - **values**: 每個 chunk 都是完整 State（包含所有欄位）
> - **updates**: 每個 chunk 只有變更的部分（格式：`{節點名: {變更欄位}}`）
