...
│                LangGraph 持久化核心概念                           │
├──────────────┬──────────────────────────────────────────────────┤
│ Thread       │ 對話/工作階段的唯一識別碼，一個 thread 包含多個     │
│              │ checkpoint，代表一次完整的對話或工作流程             │
├──────────────┼──────────────────────────────────────────────────┤
│ Checkpoint   │ 某個時間點的 State 快照（StateSnapshot），記錄圖    │
│              │ 在該時刻的完整狀態                                 │
├──────────────┼──────────────────────────────────────────────────┤
│ Super-step   │ 圖的一次「tick」——同一批被排程的節點全部執行完畢     │
│              │ 後，產生一個 checkpoint                            │
└──────────────┴──────────────────────────────────────────────────┘
```
...
```
Thread (thread_id="conv_001")
│
├── Checkpoint #0  ← Super-step -1 (輸入)
│   values: {bar: []}
│   next: (__start__,)
│
├── Checkpoint #1  ← Super-step 0 (使用者輸入處理完畢)
│   values: {foo: "", bar: []}
│   next: (node_a,)
│
├── Checkpoint #2  ← Super-step 1 (node_a 執行完畢)
│   values: {foo: "a", bar: ["a"]}
│   next: (node_b,)
│
└── Checkpoint #3  ← Super-step 2 (node_b 執行完畢)
...
建立 Thread               繼續對話              再次對話
     │                      │                      │
     v                      v                      v
invoke(input,            invoke(input,           invoke(input,
  thread_id="1")           thread_id="1")          thread_id="1")
     │                      │                      │
     v                      v                      v
┌──────────┐          ┌──────────┐           ┌──────────┐
│ CP #0    │ ──────── │ CP #2    │ ───────── │ CP #4    │
...
├───────────────┼─────────────────────────────────────────────────┤
│ parent_config │ 上一個 checkpoint 的 config，首個 checkpoint 為   │
│               │ None                                            │
├───────────────┼─────────────────────────────────────────────────┤
│ tasks         │ 該步驟要執行的任務 tuple，每個 task 有 id、name、  │
│               │ error、interrupts 等屬性                         │
└───────────────┴─────────────────────────────────────────────────┘
```
...
```
Super-step -1          Super-step 0          Super-step 1          Super-step 2
(接收輸入)             (node_a 執行)          (node_b 執行)          (結束)
     │                      │                      │                      │
     v                      v                      v                      v
┌──────────┐          ┌──────────┐          ┌──────────┐          ┌──────────┐
│ 接收     │          │ node_a   │          │ node_b   │          │ 圖結束    │
│ 使用者   │ ──────── │ 執行     │ ──────── │ 執行     │ ──────── │ next=()  │
│ 輸入     │          │          │          │          │          │          │
└──────────┘          └──────────┘          └──────────┘          └──────────┘
     │                      │                      │                      │
 ...
 source="input"        source="loop"         source="loop"         source="loop"
...
```
              ...
              ┌───────┴───────┐
              v               v
           node_a          node_b      ← 這兩個在同一個 Super-step 中
              │               │
              └───────┬───────┘
                      v
                     END

Super-step 0: node_a 和 node_b 同時執行 → 一個 Checkpoint
```
...
```
                     使用者呼叫 graph.invoke(input, config)
                                    │
                                    v
                    ┌───────────────────────────────┐
                    │   Super-step -1: 接收輸入       │
...
├───────────────────┼──────────────────────────────────────────────┤
│ 容錯 (Fault       │ 節點失敗時，從最後成功的 checkpoint 恢復，不需    │
│ Tolerance)        │ 重頭執行                                      │
├───────────────────┼──────────────────────────────────────────────┤
│ Pending Writes    │ 同一 super-step 中部分節點失敗時，成功的節點結   │
│                   │ 果會被保留，恢復時不需重新執行成功的節點          │
└───────────────────┴──────────────────────────────────────────────┘
```
