...
```mermaid
graph TD
    subgraph M1["模式 1: 工具呼叫審核"]
        A1["Agent 選擇工具"] --> A2["暫停 — 人類審核"]
        A2 --> A3["批准 / 拒絕 / 修改工具參數"]
    end

    subgraph M2["模式 2: 檢視/修改 State"]
        B1["Agent 產出結果"] --> B2["暫停 — 人類檢查"]
        B2 --> B3["修改 State 然後恢復"]
    end

    subgraph M3["模式 3: 恢復策略"]
        C1["圖在某節點中斷"] --> C2["接受 / 拒絕 / 修改 / 重播"]
        C2 --> C3["從中斷點繼續"]
    end

    subgraph M4["模式 4: 子圖中斷"]
        D1["Parent Graph"] --> D2["Subgraph"]
        D2 --> D3["暫停 — 中斷"]
        D3 --> D4["人類回應"]
    end
```
...
```mermaid
graph TD
    A["START"] --> B["plan_tool_call\nAgent 決定用哪個工具"]
    B --> C["review_tool_call\n低風險？→ 自動批准\n高風險？→ 中斷！\n人類: approve / reject / modify"]
    C -->|"should_execute = 'execute'"| D["execute_tool"]
    C -->|"should_execute = 'end'"| E1["END"]
    D --> E2["END"]
```
...
graph TD
    subgraph "父圖"
        P["prepare"] --> SG
        subgraph SG["subgraph (子圖節點)"]
            SA["sub_analyze"] --> SC["sub_confirm\ninterrupt()"]
        end
        SG --> F["final"]
    end

    SC -.->|"中斷冒泡 → 暫停"| PAUSE(("暫停"))
    PAUSE -.->|"Command(resume='approved')\n父圖 → 自動傳到子圖 sub_confirm"| SC
```
...
> **子圖中斷的重要規則：**
>
> 1. 子圖不需要自己的 checkpointer -- 自動繼承父圖的 checkpointer
> 2. 中斷會「冒泡」到父圖 -- get_state() 顯示暫停在子圖節點
> 3. Command(resume=...) 自動路由 -- 不需要指定是哪個子圖的中斷
> 4. 子圖的 State 是獨立的 -- 父圖和子圖的 State 透過共享 key 通訊
> 5. 巢狀子圖也支援 -- 中斷會一路冒泡到最外層的父圖
