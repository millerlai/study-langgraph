...
```mermaid
graph TD
    A["初始 State<br/>{foo: 1, bar: ['initial']}"]
    B["node_a 回傳: {foo: 10}<br/>覆蓋 foo"]
    C["{foo: 10, bar: ['initial']}"]
    D["node_b 回傳: {bar: ['from_node_b']}<br/>覆蓋 bar"]
    E["{foo: 10, bar: ['from_node_b']}<br/>'initial' 不見了！"]
    A --> B --> C --> D --> E
```
...
| 欄位 | Reducer | 初始值 | step_1 回傳 | step_1 後 | step_2 回傳 | 最終值 |
|------|---------|--------|------------|----------|------------|--------|
| status | 覆蓋 | "init" | "step_1_done"（取代） | "step_1_done" | "step_2_done"（取代） | "step_2_done" |
| logs | add | [] | ["Step 1"]（追加） | ["Step 1"] | ["Step 2"]（追加） | ["Step 1", "Step 2"] |
| total | add | 0 | 10（相加 0+10） | 10 | 20（相加 10+20） | 30 |
...
當多個節點在同一 super-step 中平行執行，它們可能同時更新同一個 State 欄位。**如果該欄位有 Reducer**，多個更新會透過 Reducer 合併；**如果沒有 Reducer**，LangGraph 會直接拋出 `InvalidUpdateError`。
...
```mermaid
graph TD
    subgraph superstep["同一 super-step（平行執行）"]
        research["research<br/>results: ['Research...']<br/>(不更新 status)"]
        analyze["analyze<br/>results: ['Analyze...']<br/>(不更新 status)"]
    end
    research --> merge["Reducer Merge<br/><br/>results (add): ['Research...', 'Analyze...'] 確定性合併<br/>status (無 Reducer): 維持 'init' 不變"]
    analyze --> merge
    merge --> summarize["summarize<br/>status: 'done'（非平行，安全覆蓋）"]
```
...
否則 LangGraph 會拋出 InvalidUpdateError：
  "At key 'xxx': Can receive only one value per step.
