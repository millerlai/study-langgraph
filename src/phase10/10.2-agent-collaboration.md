...
```mermaid
graph TD
    subgraph CreateAgent["使用 create_agent 建構 Supervisor"]
        Code["supervisor = create_agent(\n  model,\n  tools=[schedule_event, send_mail],\n  system_prompt='...',\n)"]
        Note["子 Agent 包裝為 tool"]
    end
```
...
graph TD
    subgraph Supervisor["Supervisor (create_agent)\ntools: analyze_data, send_alerts"]
        AD["analyze_data\n（tool 包裝）"] --> DA["Data Agent\n(create_agent)\ntools: search_db, gen_chart"]
        SA["send_alerts\n（tool 包裝）"] --> NA["Notification Agent\n(create_agent)\ntools: send_notif"]
    end
```
...
sequenceDiagram
    ...
    Note over U,B: 正確的對話歷史
    U->>A: HumanMessage: "我要退款"
    A->>A: AIMessage: tool_calls=[transfer_to_support] (必須包含)
    A->>B: ToolMessage: "已轉接到客服 Agent" (必須包含)
    B->>U: AIMessage: "好的，我來幫您處理退款..."
```
...
sequenceDiagram
    ...
    Note over U,B: 錯誤的對話歷史（缺少 ToolMessage）
    U->>A: HumanMessage: "我要退款"
    A->>A: AIMessage: tool_calls=[transfer_to_support]
    Note over A,B: 缺少 ToolMessage，LLM 會困惑！
    B->>U: AIMessage: ???
```
