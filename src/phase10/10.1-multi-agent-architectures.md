...
```mermaid
graph TD
    subgraph S4["4. Network/Router 模式"]
        Router --> NA["A"]
        Router --> NB["B"]
        Router --> NC["C"]
        NA --> Merge["合併結果"]
...
```mermaid
graph TD
    User["使用者"] --> Supervisor["Supervisor\n（路由+整合）"]
    Supervisor --> A["A\n研究"]
    Supervisor --> B["B\n寫作"]
    Supervisor --> C["C\n程式"]
```
...
graph TD
    ...
    Top --> RS["Research Supervisor"]
    Top --> CS["Content Supervisor"]
    RS --> Web["Web 搜尋"]
    RS --> Data["Data 分析"]
    CS --> Writer["寫作 Agent"]
    CS --> Editor["編輯 Agent"]
