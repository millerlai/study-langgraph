...
graph TD
    subgraph Studio["LangGraph Studio"]
        subgraph GraphViz["Graph Visualization"]
            S["START"] --> agent1["agent"]
            agent1 --> tools["tools"]
            agent1 --> END1["END"]
            tools --> agent2["agent"]
            agent2 --> END2["END"]
        end
        subgraph ExecPanel["Execution Panel"]
            Input["Input: messages"]
            State["State at each step:<br/>messages, tool_calls"]
...
```mermaid
graph TD
    subgraph Prerequisites["Studio 前置需求"]
        R1["1. LangSmith 帳號（免費註冊）"]
        R2["2. LangGraph CLI 已安裝"]
        R3["3. Agent Server 運行中（本地或已部署）"]
    end
```
...
```mermaid
graph TD
    Terminal["Terminal"] -- "langgraph dev" --> AS["Agent Server :2024"]
    AS -- "HTTP" --> Studio["Studio（瀏覽器）"]
```
...
```mermaid
graph LR
    subgraph StudioConnect["Studio 連接方式"]
        A["本地開發"] --> B["langgraph dev"] --> C["localhost:2024"]
        D["Cloud 部署"] --> E["LangSmith UI"] --> F["deployment URL"]
        G["Self-hosted 部署"] --> H["手動輸入 URL"] --> I["server URL"]
    end
```
...
```mermaid
graph TD
    subgraph GraphMode["Graph Mode 功能"]
        subgraph GP["Graph Panel"]
            GP1["顯示完整的節點與邊結構"]
            GP2["執行時高亮當前節點"]
        end
        subgraph EP["Execution Panel"]
            EP1["1. 輸入訊息"]
            EP2["2. 選擇 Thread"]
            EP3["3. 執行"]
            EP4["4. 觀察每個 node 的 state 變化"]
            EP5["5. 時間旅行到任意 checkpoint"]
        end
        subgraph SI["State Inspector"]
            SI1["messages: HumanMessage, AIMessage, ..."]
            SI2["tool_calls: name, args, id, ..."]
            SI3["metadata: step, source, writes"]
        end
    end
```
...
sequenceDiagram
    ...
    Note over U,A: Chat Mode（Thread: abc-123, Assistant: agent）
    U->>A: 台北天氣如何？
    A-->>U: 台北今天 28 度，多雲時晴，適合外出。
    U->>A: 幫我計算 123 * 456
    A-->>U: 123 * 456 = 56,088
```
...
```mermaid
graph TD
    CP0["Checkpoint 0（START）<br/>messages: HumanMessage 台北天氣如何？<br/>next: agent"]
    CP0 --> CP1["Checkpoint 1（after agent）<br/>messages: HumanMessage + AIMessage<br/>tool_calls: get_weather, city=台北<br/>next: tools"]
    CP1 --> CP2["Checkpoint 2（after tools）<br/>messages: ... + ToolMessage<br/>content: 28度，多雲時晴<br/>next: agent"]
    CP2 --> CP3["Checkpoint 3（after agent - final）<br/>messages: ... + AIMessage<br/>台北今天28度，多雲時晴！<br/>next: 執行完成"]
```
