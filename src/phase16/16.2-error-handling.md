...
```mermaid
graph TD
    subgraph ErrorStrategies["錯誤處理策略"]
        T["Transient 暫時性錯誤<br/>策略：RetryPolicy 自動重試"]
        LR["LLM-Recoverable LLM 可恢復<br/>策略：錯誤存 state，讓 LLM 重試"]
        UF["User-fixable 需使用者修復<br/>策略：interrupt 暫停等待輸入"]
        UE["Unexpected 非預期錯誤<br/>策略：讓錯誤上拋，開發者除錯"]
    end
```
...
```mermaid
graph TD
    A1["嘗試 1: 執行 → 失敗"] -- "等待 1.0 秒" --> A2["嘗試 2: 執行 → 失敗"]
    A2 -- "等待 2.0 秒（1.0 * 2.0）" --> A3["嘗試 3: 執行 → 失敗"]
    A3 --> FAIL["放棄，拋出異常"]
```

> 參數：`max_attempts=3, initial_interval=1.0, backoff_factor=2.0`
...
```mermaid
graph TD
    subgraph SuperStep["Super-step N（平行執行）"]
        NA["Node A: ✓ 成功 → pending write 已保存"]
        NB["Node B: ✓ 成功 → pending write 已保存"]
        NC["Node C: ✗ 失敗 → 需要重試"]
    end

    subgraph Resume["恢復 Super-step N"]
