...
│  def workflow(input):                            │
│      ┌─────────────────────────────────────┐     │
│      │  result_1 = task_a(x).result()      │     │
│      │                                     │     │
│      │  if condition:                      │     │
│      │      result_2 = task_b(y).result()  │     │
│      │  else:                              │     │
│      │      result_2 = task_c(z).result()  │     │
│      │                                     │     │
│      │  feedback = interrupt(...)           │     │
│      │                                     │     │
│      │  return final_result                │     │
│      └─────────────────────────────────────┘     │
│                                                  │
│  底層：共用 Pregel 執行引擎（與 Graph API 相同） │
└─────────────────────────────────────────────────┘
```
...
│            Task 執行流程                    │
│                                            │
│  future = my_task(arg)   ← 立即回傳 future │
│       │                                    │
│       v                                    │
│  result = future.result() ← 阻塞等待結果   │
│       │                                    │
│       v                                    │
│  [結果自動存入 checkpoint]                  │
│                                            │
│  若恢復執行 → 直接從 checkpoint 取回結果    │
└────────────────────────────────────────────┘
```
...
你的工作流需要...
  ...
  需要圖形視覺化？ ──────── 是 ──→ Graph API
        │ 否
        v
  複雜的多路分支與合併？ ── 是 ──→ Graph API
        │ 否
        v
  多節點共享狀態？ ──────── 是 ──→ Graph API
        │ 否
        v
  快速原型或簡單流程？ ─── 是 ──→ Functional API
        │ 否
        v
  整合既有程式碼？ ──────── 是 ──→ Functional API
        │ 否
        v
  兩者皆可，視偏好選擇
```
