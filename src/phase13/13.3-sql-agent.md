...
```
  ...
  └────────┬────────┘
           ...
           v
  ┌─────────────────┐
  │  列出資料表       │   Album, Artist, Track, Genre, ...
  └────────┬────────┘
           │
           v
  ┌─────────────────┐
  │  取得相關 Schema  │   Track(TrackId, Name, GenreId, Milliseconds, ...)
  └────────┬────────┘   Genre(GenreId, Name)
           │
           v
  ┌─────────────────┐
  │  生成 SQL 查詢    │   SELECT Genre.Name, AVG(Track.Milliseconds)
  └────────┬────────┘   FROM Track JOIN Genre ...
           │
           v
  ┌─────────────────┐
  │  檢查 SQL 正確性  │   驗證語法、邏輯錯誤
  └────────┬────────┘
           │
           v
  ┌─────────────────┐
  │  執行查詢         │   [('Sci Fi & Fantasy', 2911783), ...]
  └────────┬────────┘
           │
           v
  ┌─────────────────┐
  │  生成自然語言回答  │   "Sci Fi & Fantasy 類型的曲目平均最長..."
  └─────────────────┘
```
...
```
  ...
  ┌──────────────────┐
  │   run_query      │
  │ (帶有 interrupt) │ <--- 暫停等待人類審核
  └──────┬───────────┘
         │
    ┌────┴────┐
    │ 人類審核  │ ---> accept / edit / reject
    └────┬────┘
         │
         v
  ┌──────────────┐
  │ generate_query│ (繼續對話或結束)
  └──────┬───────┘
         │
         v
  ┌──────────────┐
